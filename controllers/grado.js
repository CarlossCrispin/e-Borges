	...
	*-----------------------------------------------*/
    getGrado: function (req, res, next) {
        ...
        pg.connect(connectionString, function(err, client, done)  {
            ...
            const query = client.query('SELECT * FROM grado ');
    ...
    postGrado : function(req,res,next){
        ...
        if(opc ==='1'){
            ...
            pg.connect(connectionString, function(err, client, done)  {
                ...
                const query = client.query(`INSERT INTO public.grado(
                    id, ngrado)
        ...
        if(opc ==='2'){
            ...
            pg.connect(connectionString, function(err, client, done)  {
                ...
                // SQL Query > Select Data
                const query = client.query(`DELETE FROM public.grado
                WHERE id=${eliminar}`);
                // Stream results back one row at a time
                query.on('row', (row) => {
                    results.push(row);
        ...
        if(opc ==='3'){
            ...
            pg.connect(connectionString, function(err, client, done)  {
                ...
                // SQL Query > Select Data
                const query = client.query(`UPDATE public.grado SET
                ngrado='${grado}' WHERE id=${editar}`);
                // Stream results back one row at a time
                query.on('row', (row) => {
                    results.push(row);
                });
